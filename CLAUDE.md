...
TTL based on `updatedAt` means a document is only deleted 24 hours *after the last message* — not 24 hours after the session started. This prevents an active conversation from being deleted mid-session.
